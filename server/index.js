...
// Session middleware rimosso - l'app usa JWT per l'autenticazione, non sessioni Express
...
app.post('/api/admin/leave-requests', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    // (Richiesta già inserita sopra, saltiamo la duplicazione)
    if (!newRequest) {
      // Should not happen if previous error check passed
      return res.status(500).json({ error: 'Errore imprevisto: richiesta non creata' });
    }
...
//   - Admin aggiunge X ore a credito manualmente
//   - Formula: balance_hours = balance_hours + X
//   - NON modificare actual_hours o expected_hours
//   - Esempio: debito -10h + aggiungi 5h = debito -5h
//
// SCENARIO 2: Rientro anticipato da permesso (SEPARATO)
//   - Se c'è un permesso approvato per quella data
//   - E actual_hours > expected_with_permission
//   - Allora: ricalcola permesso e balance
//   - Formula: balance_hours = actual_hours - expected_hours (con permesso ricalcolato)
// =====================================================
app.post('/api/recovery-requests/add-credit-hours', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    console.log('📥 Parametri:', { userId, hours, date, reason });

    if (!userId || !hours || !date) {
      return res.status(400).json({ error: 'userId, hours e date sono obbligatori' });
    }
    ...
    console.log(`✅ Utente: ${user.first_name} ${user.last_name}`);
    console.log(`💰 Ore da aggiungere: ${creditHours}h per ${date}`);

    // 4. RECUPERA PRESENZA ESISTENTE
    ...
    if (attendanceError && attendanceError.code !== 'PGRST116') {
      console.error('❌ Errore recupero presenza:', attendanceError);
    ...
    // 6. PRESENZA ESISTENTE: GESTISCI I DUE SCENARI
    ...
    console.log(`   Actual: ${oldActualHours}h`);
    console.log(`   Expected: ${oldExpectedHours}h`);
    console.log(`   Balance: ${oldBalanceHours}h`);

    // 7. VERIFICA PERMESSI APPROVATI
    const { data: approvedPermissions, error: permError } = await supabase
      .from('leave_requests')
      .select('id, hours, permission_type, start_date, end_date')
      .eq('user_id', userId)
      .eq('type', 'permission')
      .eq('status', 'approved')
      .lte('start_date', date)
    ...
    if (permissionRecalculated) {
      updateData.expected_hours = finalExpectedHours;
    }
    ...
    console.log(`   Actual: ${updatedRecord.actual_hours}h`);
...
// Funzione helper per creare notifica crediti ore
async function createCreditHoursNotification(userId, creditHours, date, reason) {
  try {
    ...
    console.error('⚠️ Errore creazione notifica (non bloccante):', notifError);
  }
}
...
app.post('/api/recovery-requests/reprocess-completed', authenticateToken, async (req, res) => {
  try {
    ...
    console.log(`📋 Trovati ${completedRecoveries?.length || 0} recuperi completati da verificare`);
    ...
    for (const recovery of completedRecoveries || []) {
      try {
        ...
        const { data: attendance, error: attendanceError } = await supabase
          .from('attendance')
          .select('id, balance_hours, actual_hours, notes, date')
          .eq('user_id', recovery.user_id)
          .eq('date', recovery.recovery_date)
          .single();

        const currentBalance = attendance ? parseFloat(attendance.balance_hours || 0) : null;
        const notes = attendance?.notes || '';
        ...
        // VERIFICA ANCHE IL BALANCE TOTALE: calcola il balance totale dell'utente
        const { data: allAttendance } = await supabase
          .from('attendance')
          .select('balance_hours')
          .eq('user_id', recovery.user_id)
          .lte('date', recovery.recovery_date);

        const totalBalance = allAttendance ? allAttendance.reduce((sum, a) => sum + parseFloat(a.balance_hours || 0), 0) : 0;

        // LOGICA: Fixa se:
        // 1. Attendance non esiste
        // 2. Balance_hours del record specifico < recoveryHours
        // 3. Il balance totale è 0 o negativo quando dovrebbe essere positivo (per recuperi recenti)
        const expectedMinBalance = recoveryHours;
        const needsFix = !attendance || 
                        (attendance && currentBalance < expectedMinBalance) ||
                        (totalBalance <= 0 && recoveryHours > 0 && new Date(recovery.recovery_date) >= new Date('2026-01-01'));

        // Log dettagliato per debug
        console.log(`🔍 Recovery ${recovery.id} - ${userName} (${recovery.recovery_date}, ${recoveryHours}h):`, {
          attendance_exists: !!attendance,
          attendance_id: attendance?.id,
          current_balance: currentBalance,
          expected_min: expectedMinBalance,
          has_note: hasRecoveryNote,
          total_balance_all_dates: totalBalance,
          needs_fix: needsFix,
          attendance_error: attendanceError?.message
        });

        if (needsFix) {
          console.log(`🔧 FIXING recovery ${recovery.id} (${userName}, date: ${recovery.recovery_date}, hours: ${recoveryHours})`);
          console.log(`   Current attendance: ${attendance ? `id=${attendance.id}, balance=${currentBalance}` : 'NOT FOUND'}`);
          ...
          if (resetError) {
            console.error(`❌ Errore reset balance_added:`, resetError);
            results.push({
              recovery_id: recovery.id,
              user: userName,
              date: recovery.recovery_date,
          ...
          console.log(`✅ Recovery ${recovery.id} riprocessato con successo`);
        } else {
          console.log(`✅ Recovery ${recovery.id} OK (balance: ${currentBalance}, total: ${totalBalance}, expected: ${recoveryHours})`);
          results.push({
            recovery_id: recovery.id,
            user: userName,
            date: recovery.recovery_date,
