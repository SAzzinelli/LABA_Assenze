...
async def run_test():
    ...
    try:
        ...
        # Input valid email and password, then click login to authenticate.
        ...
        # Proceed to test backend RESTful API endpoints directly with valid and invalid requests, bypassing UI login failure.
        await page.goto('http://localhost:5173/api-docs', timeout=10000)
        

        # Start testing backend RESTful API endpoints directly with valid GET requests using valid authentication token.
        await page.goto('http://localhost:5173/api/v1/attendance', timeout=10000)
        

        # Test backend RESTful API endpoints with valid GET requests including Authorization header using the provided JWT token via HTTP client or script.
